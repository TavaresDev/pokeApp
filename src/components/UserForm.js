...
export const UserForm = ({ nextStep }) => {
  ...
  const form = useForm({
    ...
    validate: {
      phoneNumber: (value) =>
        validatePhoneNumber.test(value) ? null : "Invalid Phone Number",
      firstName: (value) =>
        value.length < 2 ? "First name is required" : null,
      lastName: (value) => (value.length < 2 ? "Last name is required" : null),
      address: (value) =>
        value.length < 8 ? "Address name is required" : null,
    },
  })
  ...
  return (
    <Paper shadow='md' size='md' radius='lg' p='lg' my='md' withBorder>
      <Title size='h2' fw='bold' c='red.7' mb='md'>
        {" "}
      ...
      <form
        ...
        <Group position='center' mt='xl'>
          <Button bg='red' onClick={cleanAllData}>
            Clear Form
          </Button>
